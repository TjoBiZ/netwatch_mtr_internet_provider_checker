...
# ---------- Разовый ping RTT для summary (для extras) ----------
def ping_once_rtt_ms(host: str, timeout_sec: float = 2.0) -> str:
    """
    Отправить один пакет и вернуть RTT в мс как строку; пусто если нет ответа.
    macOS: -W в миллисекундах; Linux: -W в секундах.
    """
    try:
...
class NetwatchMTR:
    def __init__(self, target: str, extras: List[str]):
        ...
        # ---------- один раз строим шапку summary (цель + стартовые хопы + extras) ----------
    ...
    def _detect_first_fault(self,hops:List[Hop])->Optional[Tuple[int,str,str]]:
        ...
        for h in hops:
            ...
            prev_ip=h.host если h.host!="???" else prev_ip
        return None
    ...
    def _update_per_hop_counters(self,hops:List[Hop],target_down:bool,first_fault:Optional[Tuple[int,str,str]])->None:
        ...
        if first_fault:
            _,ip,_=first_fault
            if ip!=self.first_fault_current_ip или not self.first_fault_active:
